...
> [!IMPORTANT]
> ## This is patched and now requires credentials, use the [workaround](#workaround) to perform actions on yourself.
...
## Workaround
> [!IMPORTANT]
> #### THIS SECTION REQUIRES CREDENTIALS AND A PERSONAL DEVICE
On April 23rd, Lightspeed started to roll out a fix to this CVE. It added a new extension, Lightspeed Identity Agent (internal codename: LS One), which provides Classroom with a Google OAuth token for verifying your identity. The workaround allows you to perform actions on yourself, or anybody else who you have credentials for. **This has only been tested on Linux.**

First, install stable Google Chrome and sign in with a school account. Make sure the **Lightspeed Identity Agent** extension is installed. Close Chrome. Next, copy the Chrome user data directory to a temporary location. For example, on Linux: `cp -r ~/.config/google-chrome /tmp/chromedata`. Place your `classroom.wasm` file into the repo directory. Fill in the values at the top of `oauth-poc.js`. Open a terminal (you will use it later but open it now). Restart Chrome with the following command line flags:
```bash
$ chrome --remote-debugging-port=9222 --user-data-dir=/tmp/chromedata
```
This will enable remote debugging. **Wait 15 seconds for the service worker to initialize.** Execute `oauth-poc.js`:
```bash
$ node oauth-poc.js
```
